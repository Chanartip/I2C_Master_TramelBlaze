...
module Debounce(clk, rst, db_in, db_out);
   ...
   always@(*) begin
      ...
      case({ps_state, db_in})
         {   zero, 1'b0}: {ns_state, ns_out} = {   zero, 1'b0};
         {wait1_1, 1'b0}: {ns_state, ns_out} = {   zero, 1'b0};
         {wait1_2, 1'b0}: {ns_state, ns_out} = {   zero, 1'b0};
         {wait1_3, 1'b0}: {ns_state, ns_out} = {   zero, 1'b0};
			
         {   zero, 1'b1}: {ns_state, ns_out} = 
										 {wait1_1, 1'b0};
         {wait1_1, 1'b1}: {ns_state, ns_out} = 
                       (tick)? {wait1_2, 1'b0}:{wait1_1, 1'b0};         
         {wait1_2, 1'b1}: {ns_state, ns_out} = 
                       (tick)? {wait1_3, 1'b0}:{wait1_2, 1'b0};          
         {wait1_3, 1'b1}: {ns_state, ns_out} = 
                       (tick)? {    one, 1'b1}:{wait1_3, 1'b0};
                       
							  
         {    one, 1'b0}: {ns_state, ns_out} = 
										 {wait0_1, 1'b1};        
         {wait0_1, 1'b0}: {ns_state, ns_out} = 
                       (tick)? {wait0_2, 1'b1}:{wait0_1, 1'b1};                       
         {wait0_2, 1'b0}: {ns_state, ns_out} =
                       (tick)? {wait0_3, 1'b1}:{wait0_2, 1'b1};                       
         {wait0_3, 1'b0}: {ns_state, ns_out} = 
                       (tick)? {   zero, 1'b0}:{wait0_3, 1'b1};
         
			{    one, 1'b1}: {ns_state, ns_out} = {    one, 1'b1};
         {wait0_1, 1'b1}: {ns_state, ns_out} = {    one, 1'b1};
         {wait0_2, 1'b1}: {ns_state, ns_out} = {    one, 1'b1};
         {wait0_3, 1'b1}: {ns_state, ns_out} = {    one, 1'b1};
