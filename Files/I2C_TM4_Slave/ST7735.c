...
void ST7735_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
	...
	if(ch == 8)
	{
		if((StX == 0) && (StY == 0)) // Setting boundaries for StX == 0, StY == 0
		{
			StX = 0;
			StY = 0;
		}
		else if(StX == 0) // Going back to the last line when StY > 0
		{
			StY = StY - 1;
			StX = 20;
			ST7735_DrawCharS(StX*6,StY*10, ' ',ST7735_WHITE,ST7735_BLACK, 1);
		}
		else
		{
			StX--;
  ...
  {
		ST7735_DrawCharS(StX*6,StY*10,ch,ST7735_WHITE,ST7735_BLACK, 1);
		StX++;
		if(StX>20)
		{
			StY++; StX = 0;
